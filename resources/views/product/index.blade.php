    ...
    },
    {
        ...
        <div class="items-start justify-between md:flex">
            <div class="max-w-lg">
                ...
                <p class="text-gray-600 mt-2">Products Details in here!</p>
            </div>
            <div class="mt-3 md:mt-0">
                <a href="{{ URL::tokenRoute('product.form') }}" class="inline-block px-4 py-2 text-white duration-150 font-medium bg-indigo-600 rounded-lg hover:bg-indigo-500 active:bg-indigo-700 md:text-sm">Add product</a>
            </div>
        </div>
            ...
            <table class="w-full table-auto text-sm text-left">
                ...
                <tbody class="text-gray-600 divide-y">
                    @foreach($products as $product)
                        <tr>
                            <td class="pr-6 py-4 whitespace-nowrap">{{ $product['name'] }}</td>
                            <td class="pr-6 py-4 whitespace-nowrap">{{ $product['description'] }}</td>
                            <td class="pr-6 py-4 whitespace-nowrap">{{ $product->collection['name'] }}</td>
                            <td class="pr-6 py-4 whitespace-nowrap">
                                <span :class="`px-3 py-2 rounded-full font-semibold text-xs ${$collection['status_id']  === 1 ? 'text-green-600 bg-green-50' : 'text-blue-600 bg-blue-50'}`" x-text="item.status">{{ $product['status_id']== 1 ? 'Active' : 'Inactive' }}</span>
                            </td>
